...
GL.allPlays = function(hand,lp,h){
  ...
  if(lt==='single'){var chaR=lp.cards[0].r,chaCards=rc[chaR]||[];if(chaCards.length>=2)ps.push({type:'cha',rk:999,cards:chaCards.slice(0,2)})}
  if(lt==='pair'){var dianR=lp.cards[0].r,dianCards=rc[dianR]||[];if(dianCards.length>=1)ps.push({type:'dian',rk:999,cards:[dianCards[0]]})}
  return ps;
};
...
GL.execPlay = function(G,pi,play){
  ...
  G.lp=play;G.lpp=pi;G.ps=0;G._fromCha=false;
...
GL.canCha = function(G,srcPlayer,chaRank){
  for(var i=1;i<=3;i++){var p=(srcPlayer+i)%4;if(G.dn[p])continue;if(G.hd[p].filter(function(c){return c.r===chaRank}).length>=2)return p}
  return -1;
};

GL.canDian = function(G,chaPlayer,chaRank){
  for(var i=1;i<=3;i++){var p=(chaPlayer+i)%4;if(G.dn[p]||p===chaPlayer)continue;if(G.hd[p].some(function(c){return c.r===chaRank}))return p}
  return -1;
};
