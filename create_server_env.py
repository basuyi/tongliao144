#!/usr/bin/env python3
"""Create Lightweight Application Server using environment variables for credentials."""
import sys
import os
import json
import time
...
def create_lightweight_server(ak_id, ak_secret, region_id='cn-beijing'):
    ...
    request.add_query_param('PlanId', 'swas.s2.c2.m20.b3.f60')
...
if __name__ == '__main__':
    ak_id = os.getenv('ALIBABA_CLOUD_ACCESS_KEY_ID') or os.getenv('OSS_ACCESS_KEY_ID')
    ak_secret = os.getenv('ALIBABA_CLOUD_ACCESS_KEY_SECRET') or os.getenv('OSS_ACCESS_KEY_SECRET')
    
    if not ak_id or not ak_secret:
        print("[ERROR] Missing credentials.")
        print("Please set environment variables:")
        print("  set ALIBABA_CLOUD_ACCESS_KEY_ID=your_key_id")
        print("  set ALIBABA_CLOUD_ACCESS_KEY_SECRET=your_key_secret")
        sys.exit(1)
    
    create_lightweight_server(ak_id, ak_secret)
